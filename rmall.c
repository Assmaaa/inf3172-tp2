...
int main(int argc, char* argv[]){
    if(argc != 2)
        fprintf(stdout, "<rmall> prend un chemin relatif/absolu en parametre\n");
    else
    {
        if (!argv[1] || access(argv[1], R_OK)) perror("ce parametre n'est pas accessbile");
        else if (nftw(argv[1], &remove_directory, 1, FTW_DEPTH)) perror("error occurred in ftw");
    }
    return EXIT_SUCCESS;
}
